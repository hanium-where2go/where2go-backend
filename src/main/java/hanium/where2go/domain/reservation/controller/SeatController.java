...
public class SeatController {
    ...
    private static String restaurantKey; // 전역변수로 선언
    ...
    public Seats message(Seats seats) {
        try {
            restaurantKey = "restaurantId: " + seats.getRestaurant();
            // Seats 객체를 JSON 문자열로 직렬화하여 /sub 주제로 전송
            redisUtil.set("storeStatus", "OPEN",2 *60*60*1000); // 영업 상태
            redisUtil.set(restaurantKey, Integer.toString(seats.getSeat()), 2 * 60 * 60 * 1000); // 2시간(120분) 동안 유효

            // Redis에서 가게 상태 가져오기
            String storeStatus = (String) redisUtil.get("storeStatus");

            // Redis에서 좌석 수 가져오기
            String seatCount = (String) redisUtil.get(restaurantKey);

            seats.setStatus(storeStatus);
            seats.setSeat(Integer.valueOf(seatCount));
            seats.setRestaurant(seats.getRestaurant());
    ...
    public void closeStore() {
        try {
            // 가게 상태를 CLOSED로 업데이트
            redisUtil.set("storeStatus","CLOSED",2*60*60*1000);
    ...
    public String closeOperationStatus() {
        try {
            String storeOperationStatus = (String) redisUtil.get("storeStatus");
